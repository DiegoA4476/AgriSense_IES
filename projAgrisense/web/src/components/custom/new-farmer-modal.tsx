...
export function NewFarmerModal({ onSuccess }: NewFarmerModalProps) {
  ...
  const handleCreate = async () => {
    try {
      const response = await createFarmer.mutateAsync({ firstName, lastName, email });
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent className="p-0 overflow-hidden">
          ...
          <AlertDialogDescription className="flex flex-col gap-3 pl-6 pr-6 pt-2 w-full text-[#000000]">
            {showPassword ? (
              <div className="flex flex-col gap-3">
                <p className="text-sm">Farmer created successfully. Please provide this temporary password to the farmer:</p>
                <div className="bg-gray-100 p-4 rounded border border-gray-300">
                  <p className="font-mono text-lg font-bold text-center break-all">{password}</p>
                </div>
                <p className="text-xs text-gray-600">The farmer will be required to change this password on first login.</p>
              </div>
            ) : (
        ...
        <AlertDialogFooter className="pl-6 pr-6 pb-6 flex flex-row justify-between! w-full!">
          {showPassword ? (
            <AlertDialogAction onClick={handleClose} className="w-full bg-[#16A34A] cursor-pointer">
              Close
            </AlertDialogAction>
          ) : (
            <>
              ...
              <AlertDialogAction 
                ...
                disabled={!firstName || !lastName || !email || createFarmer.isPending}
              >
